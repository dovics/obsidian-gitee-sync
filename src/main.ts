...
import { GiteeSyncSettings, DEFAULT_SETTINGS } from "./settings/settings";
import GiteeSyncSettingsTab from "./settings/tab";
...
export default class GiteeSyncPlugin extends Plugin {
  ...
  async onUserEnable() {
    if (
      this.settings.giteeToken === "" ||
      this.settings.giteeOwner === "" ||
      this.settings.giteeRepo === "" ||
      this.settings.giteeBranch === ""
    ) {
  ...
  async onload() {
    ...
    this.addSettingTab(new GiteeSyncSettingsTab(this.app, this));
  ...
  updateStatusBarItem() {
    ...
    this.statusBarItem.setText(`Gitee: ${state}`);
  }
